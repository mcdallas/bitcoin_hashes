...
impl<T: Hash> HmacEngine<T> {
    /// Construct a new keyed HMAC with the given key
    pub fn new(key: &[u8]) -> HmacEngine<T> {
        ...
        io::Write::write(&mut ret.iengine, &ipad[..T::block_size()]).expect("writing ipad");
        io::Write::write(&mut ret.oengine, &opad[..T::block_size()]).expect("writing opad");
        ret
    }
}
...
impl<T: Hash> Hash for Hmac<T> {
    ...
    fn from_engine(mut e: HmacEngine<T>) -> Hmac<T> {
        ...
        io::Write::write(&mut e.oengine, &ihash[..]).expect("writing to outer engine");
...
mod tests {
    ...
    fn test() {
        ...
        for test in tests {
            ...
            engine.write(&test.input).expect("writing input");
